...
通爻（ToWow）的协商流程中有 6 个关键 LLM 调用点（称为 Skill），它们负责：把用户模糊的想法变成清晰的需求、让 Agent 生成有洞察力的响应、把多个响应聚合成有创造性的方案。

**Prompt 的质量直接决定了用户感知到的系统质量。** 一个好的 Prompt 能让 LLM 发现"意想不到的价值组合"；一个差的 Prompt 只会让 LLM 做表面匹配。

但有一个核心约束：**代码保障 > Prompt 保障**（设计原则 0.5）。能用代码逻辑（状态机、等待屏障、观察遮蔽）保证的行为，就不应该依赖 Prompt。Prompt 的职责是"提供智能"，不是"保证行为"。

H2 的工作是：在严守这个边界的前提下，系统性地优化 6 个 Skill 的 Prompt，并建立可复用的评估基准和设计模式。

```
[H2: Prompt 工程研究（你在这里）]
    ├── 优化 6 个 Skill 的 Prompt（V1 → V2）
    ├── 建立评估基准（可被 H4 复用）
    ├── 缓解第一提案偏见（代码层 + Prompt 层联合）
    └── 提炼设计模式手册（永久资产）
```
...
- 所有产出提交到 `research/H2_prompt_engineering/` 目录
...
**做完了（基本合格）**：
- 6 个 Skill 都有 V2 Prompt，且有 V1 vs V2 的对比数据
- 评估基准可被后续迭代复用
- "代码保障 vs Prompt 保障"的边界在每个 Skill 中明确标注

**做得好（超出预期）**：
- Prompt 设计模式手册有 6+ 个可复用模式，每个有正例/反例
- 第一提案偏见的缓解方案有定量验证数据
- 发现了架构文档中未提及的 LLM 偏见或问题

**做得出色（产生额外价值）**：
- Prompt 设计模式手册可独立发表为技术博客（面向 AI Agent 社区）
- 发现了"代码保障 vs Prompt 保障"的通用决策框架，适用于所有 Agent 系统
- V2 Prompt 在真实用户测试中获得明显更好的反馈
- 评估基准被 H4（最小验证实验）直接采用
...
### 提交位置
`research/H2_prompt_engineering/`
