...
| 画像演化 | 投影即函数（数据源更新 + 重新投影） | 通爻不维护 Profile 状态，协作数据回流到数据源，重新投影时自然反映（见 Section 7.1.6） |
...
**传统产品** 每个都要自建：用户管理 → 匹配算法 → 协调逻辑 → 通知 → 支付 → 评价 → 分析。垂直不同，骨架相同。
...
其余全部由协议层提供。**产品开发从"智能工程"变成"场景定义"。**

### 13.2 协商单元 API 边界

产品开发者与协议层交互的唯一接口。产品层不需要知道 HDC、Center、共振检测等任何内部机制。

**产品层 → 协议层（调用方向）**：

| API | 作用 |
|-----|------|
| `create_scene(config)` | 创建场景（Template、k*、access 等） |
| `register_agent(scene_id, profile_data)` | 在场景中注册 Agent |
| `submit_demand(scene_id, user_id, intent)` | 提交需求 |
| `confirm_formulation(demand_id, confirmed_text)` | 确认丰富化后的需求 |
| `user_action(negotiation_id, action)` | 用户对方案的操作（接受/修改/拒绝） |

**协议层 → 产品层（事件推送，全量）**：

| 事件 | 数据 |
|------|------|
| `formulation.ready` | 丰富化结果，等用户确认 |
| `resonance.activated` | 被激活的 Agent 数量和共振分数分布 |
| `offer.received` | 单个 Offer 内容（逐个推送） |
| `barrier.complete` | 所有 Offer 收集完毕 |
| `center.tool_call` | Center 的每次工具调用（追问/发现/子需求...） |
| `plan.ready` | 最终方案（plan 文本 / Machine 草案） |
| `sub_negotiation.started` | 子协商启动 |
| `execution.progress` | 执行进展（WOWOK 事件） |
| `echo.received` | 回声信号 |

**设计原则**：事件流全量推送，产品层自行选择展示哪些。协议不预判什么重要什么不重要。

### 13.3 场景的生命周期定位

**场景是前期的商业工具，不是永久的架构概念。**

```
V1: 场景（手动创建，组织者运营）→ 用户获取 + 数据积累
V2: Service Agent 聚类 → 市场自然涌现（标准需求的快速响应集群）
V3: 场景溶解，市场 + 开放网络 → 响应范式完全体
```

场景存在的目的：
1. **商业入口**：通过一个一个场景单点突破（黑客松、创业社区、企业内部）
2. **数据收集**：Template 收集参与者信息，丰富 Profile
3. **价值证明**：每个成功场景都是协议价值的案例

当网络成熟后，场景不再是必要的——Agent 在开放网络中自由共振，市场从重复交互中涌现（见 Section 1.4 场景 vs 市场）。

### 13.4 进阶洞察：场景即 Agent

> 标记为 V2+ 探索方向。

如果投影是基本操作，场景本身也可以被投影为 HDC 向量：
...
三者都是投影，都可以参与共振。这意味着：
- 用户不搜索场景，合适的场景自己浮现（场景跟用户共振）
- 场景之间也可以共振（一个场景的缺口触发跟另一个场景的关联）
- 场景可以从交互中涌现，不一定需要组织者手动创建

**V1 不实现，但数据结构预留兼容性。**

### 13.5 协议层学习机制（Protocol-Level Learning）

> 标记为 V2+ 探索方向。

回声机制（Section 11.3）解决 Agent 层面的学习。但协议本身也需要从产品运行中学习：

**跨场景追溯链聚合**：
- 某类场景的最优 k* → 推荐默认值
- 哪些 formulation 追问模式有效 → 改进默认策略
- Center 工具调用模式 → 优化 prompt（通过 SkillPolisher）
- 失败的场景（最有价值的信号）→ 识别协议的薄弱点

**关键**：失败比成功更有价值。追溯链结构（Section 11.7）已支持事后分析，但"协议怎么从分析中自动改进"是新课题。
...
*最后更新：2026-02-08（第十二轮：新增 Section 13 "场景即产品"产品范式——协商单元 API 边界、场景生命周期定位、场景即 Agent 探索方向、协议层学习机制；修复 Section 6.1.9 技术选型表画像演化项与 7.1.6 不一致）*
