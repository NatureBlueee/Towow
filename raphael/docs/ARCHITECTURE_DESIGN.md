...
**单次调用**，不是多步 pipeline。一次 LLM 调用中完成理解、判断、决策。

**最多 2 轮**（代码控制）：研究表明多轮迭代平均效果 -3.5%，错误放大 4.4x（Google DeepMind, 2025）。超过 2 轮 → 程序层强制要求出方案。
...
⑤ 等待屏障（Barrier）——代码控制
   ...
   超时 → 标记未返回的为"退出"
     ↓
⑥ 组装完整输入，调用 Center       → CenterCoordinatorSkill
   （含用户偏好/约束作为 context）     （1次调用）
     ↓
⑦ 解析 Center 的输出（代码）
     ├─ "plan" → 输出方案，结束
     ...
     │                   结果返回 → 回到 ⑥
     └─ "has_gap" → 调用                → GapRecursionSkill
                     生成子需求 → 回到 ①（递归）
     ↓
⑧ 轮次计数器（代码控制）
   超过 2 轮 → 强制要求 Center 出方案
```
