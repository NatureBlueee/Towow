#!/usr/bin/env python3
"""
SIM-002: 3-person crystallization simulation (v1 catalyst prompt)

Same scenario as SIM-001 (Lina + 赵维 + Maya, manufacturing CEO trigger).
Only change: catalyst prompt v0 → v1.

Purpose: Validate v1 catalyst prompt fixes:
  H5: Forced output format → reliable convergence self-check
  H6: Prohibitive constraints → zero action recommendations
  H7: Translation priority + examples → more cross-semantic translations
  H8: Restrictions don't degrade output quality

Usage:
    cd backend && source venv/bin/activate
    TOWOW_ANTHROPIC_API_KEY=sk-ant-... python ../tests/crystallization_poc/simulations/sim002_3person_v1/run_sim.py
"""
...
SIM001_DIR = BASE_DIR.parent / "sim001_3person"
...
def load_trigger() -> str:
    """Load trigger event from SIM-001, excluding analysis sections."""
    content = load_text(SIM001_DIR / "trigger.md")
    lines = content.split("\n")
    result = []
    for line in lines:
...
def build_endpoint_prompt(
    ...
    """Same endpoint prompt as SIM-001 — no changes."""
...
def check_convergence(observation: str) -> bool:
    """Check for convergence signals in v1 format.

    v1 catalyst output ends with:
      ## 收敛判断
      新发现数量：N
      收敛状态：本轮没有新发现 / 本轮有 N 个新发现
    """
    markers = [
        "本轮没有新发现",
        "没有新的发现",
...
def run_simulation():
    ...
    # KEY CHANGE: v1 catalyst prompt
    catalyst_system = extract_prompt_block(PROMPTS_DIR / "catalyst_v1.md")

    print("=== SIM-002: 3-Person Crystallization (v1 Catalyst) ===")
    print(f"Participants: {', '.join(profiles.keys())}")
    print(f"Endpoint model: {ENDPOINT_MODEL}")
    print(f"Catalyst model: {CATALYST_MODEL}")
    print(f"Max rounds: {MAX_ROUNDS}")
    print(f"Catalyst prompt: v1")
    print()
    ...
    for round_num in range(1, MAX_ROUNDS + 1):
        ...
        # Call catalyst agent with v1 prompt
        print(f"  Calling catalyst (v1)...", end="", flush=True)
    ...
    # Run plan generator (same as SIM-001)
