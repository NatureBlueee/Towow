...
- 协议层推送事件（全量，不预判重要性）
- **我决定展示哪些、怎么展示**
- 用户通过 UI 调用 5 个 API（submit_demand、confirm_formulation 等）
...
- **协商过程可视化**：让用户"看到"协商在进行——formulation 结果、共振激活、Offer 逐个到达、Center 思考过程
...
| 事件 | UI 含义 |
|------|---------|
| `formulation.ready` | 展示丰富化结果，请求用户确认 |
...
- 所有 9 种事件都能正确接收和解析
- 用户操作（提交需求、确认 formulation、操作方案）能正确调用 API
- UI 实时更新，不需要刷新页面
