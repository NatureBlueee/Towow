...
```
1. 读 tests/crystallization_poc/state.json
...
```bash
python3 tests/crystallization_poc/simulations/real/assemble_prompts.py \
    --config run_NNN/config.json
```

**Phase 3 (RUN) 每阶段出口门禁**：每个阶段（催化、端侧、方案、交付）完成后，运行验证：
```bash
python3 tests/crystallization_poc/simulations/real/validate_names.py \
...
| 脚本 | 位置 | 用途 |
|------|------|------|
| `assemble_prompts.py` | `tests/crystallization_poc/simulations/real/` | 预组装所有 prompt，代码级名称绑定 |
| `validate_names.py` | `tests/crystallization_poc/simulations/real/` | 校验输出中的名称一致性 |

---

## 状态文件

**位置**: `tests/crystallization_poc/state.json`
  ...
  "runs": [
    {
      ...
      "dir": "tests/crystallization_poc/simulations/real/run_001/",
      "summary_200w": "...",
      "evaluation": {
        "user_verdict": "...",
        "identified_factors": [
...
超过 10 个 run 后，旧 run 的 `summary_200w` 压缩为一行，详情归档到 `tests/crystallization_poc/simulations/real/iteration_log.md`。
...
Phase 1~N: 每轮（共享任务列表驱动）
  ...
  ┌─ Teammate [P03] → 读 Profile + formulation + 上轮催化 → 写 round_N_P03.md → 标记完成
...
Phase Final: Plan Generator
  Lead spawn 或复用 teammate → 读所有轮次输出 + formulation → 写 plan.md
```
...
可以只跑 Phase 0（测 formulation）或只跑一轮（测 endpoint + catalyst），不必跑完整流程。prompt 迭代阶段主要用这种方式。
...
**用户做什么**: 确认 formulation → 按"开始" → 每轮可通过 Shift+Down 介入任意 teammate
...
**Prompt 来源**: `tests/crystallization_poc/prompts/formulation_v1.md`（自包含，包含概念定义+思维链）

**Spawn prompt 模板**:
```
你是 Formulator。你的任务是将原始需求编码为四参数张力结构。

读取 prompt 文件: {formulation_prompt_path}
...
**Prompt 来源**: `tests/crystallization_poc/prompts/endpoint_v1.md`
...
**输入**: Profile + formulation + 上轮催化（通过任务描述指定路径）
...
**Prompt 来源**: `tests/crystallization_poc/prompts/catalyst_v2.md`
...
**输入**: 本轮所有 endpoint 输出 + formulation + 历史催化
...
**Prompt 来源**: `tests/crystallization_poc/prompts/plan_generator_v0.md`
...
Stage 0: FORMULATE
  ...
  └─ 跳过条件: config.pipeline.skip_formulation=true 且 formulated_demand_file 已指定
...
位于 `tests/crystallization_poc/simulations/real/pipeline_config_template.json`。
...
Agent Teams 模式下每个 teammate 是独立 Claude Code 实例，token 消耗随 teammate 数量线性增长。一个 4 轮 run（3 endpoint + 1 catalyst + formulation + plan）约消耗 6 个 teammate 的 token。
...
| `towow-eng-prompt` | 深度 prompt 工程时加载 |
| `arch` | 评估发现设计层问题时升级 |
...
### 核心状态
- `tests/crystallization_poc/state.json` — 实验状态（单一真相源）

### Prompt 版本
- `tests/crystallization_poc/prompts/formulation_v1.1.md` — 当前 formulation prompt
- `tests/crystallization_poc/prompts/catalyst_v2.1.md` — 当前催化 prompt
- `tests/crystallization_poc/prompts/endpoint_v2.md` — 当前端侧 prompt
- `tests/crystallization_poc/prompts/plan_generator_v0.md` — 当前方案生成 prompt
- `tests/crystallization_poc/prompts/delivery_v0.md` — 当前交付 prompt

### 管道工具
- `tests/crystallization_poc/simulations/real/assemble_prompts.py` — Prompt 预组装器（代码级名称绑定）
- `tests/crystallization_poc/simulations/real/validate_names.py` — 名称一致性校验门
- `tests/crystallization_poc/simulations/real/pipeline_config_template.json` — 管道配置模板
- `tests/crystallization_poc/simulations/real/run_real.py` — 种子控制批量运行脚本（API 模式备选）
- `tests/crystallization_poc/simulations/real/test_delivery.py` — Delivery 单独测试脚本
