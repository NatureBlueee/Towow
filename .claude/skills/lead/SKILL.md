...
**思维模式**：架构师思维（`arch` skill 的思考方式）
- 本质与实现分离
- 复杂性从简单规则生长
- 验证思维：能否递归？规模 10x 会怎样？
- 变更思维：契约 vs 实现，变更传播
...
**思维模式**：工程 Leader 思维（`towow-eng` skill 的思考方式）
- 契约优先
- 按模块边界拆，不按功能流程拆
- 跨 Track 数据流显式分配
- 接缝验证
...
**调度**：根据改动领域，调用对应的专才 skill：

| 领域 | 专才 Skill |
|------|-----------|
| 全栈实现 | `towow-dev` |
| 状态机/编排 | `towow-eng-orchestrator` |
| LLM/Prompt | `towow-eng-prompt` |
| HDC/向量 | `towow-eng-hdc` |
| 前端 | `towow-eng-frontend` |
| 测试 | `towow-eng-test` |
...
| 改动类型 | 起始阶段 | 例子 |
|----------|----------|------|
| 新功能 / 新方向 | 阶段 ① 讨论 | MCP 入口、新的 Adapter 类型 |
| 已知问题修复（有 Issue 诊断） | 阶段 ④ 实现方案 | Issue 001 Formulation 修复 |
| 小修改（≤3 文件，无契约变更） | 阶段 ⑤ 直接实现 | 修 typo、调整 log 级别 |
| 架构方向讨论（不产出代码） | 阶段 ① 讨论 → ② ADR | 技术选型、协议演进 |
